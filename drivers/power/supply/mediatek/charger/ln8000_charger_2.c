...
static int ln8000_init_irq(struct ln8000_info *info)
{
	...
	ret = gpio_request(info->irq_gpio, "ln8000");
	if (ret < 0) {
		ln_err("fail to request GPIO(%d)   %d\n", info->irq_gpio, ret);
		return ret;
	}

	ret = gpio_direction_input(info->irq_gpio);
	if (ret < 0) {
		ln_err("fail to set GPIO%d as input pin(%d)\n", info->irq_gpio, ret);
		return ret;
	}

	info->irq = gpio_to_irq(info->irq_gpio);
	if (info->irq <= 0) {
		ln_err("irq mapping fail\n");
		return 0;
	}
	...
	ln8000_write_reg(info, LN8000_REG_INT1_MSK, mask);        /* read clear int_reg */
	ret = ln8000_read_int_value(info, &int_reg);
	if (IS_ERR_VALUE((unsigned long)ret)) {
		ln_err("fail to read INT reg (ret=%d)\n", ret);
		return IRQ_NONE;
...
static int ln8000_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	hardwareinfo_set_prop(HARDWARE_SUB_CHARGER_SLAVE_IC_INFO, "LN8000_CHARGER_SLAVE");
